...
// config -- tune these
// ---------------------------------------------------------------------
static int MAX_NODES = 2000;   // node cap per tree
static int MAX_TREES = 1;      // trees allocated (only tree 0 is grown for now)
...
static const double MARKER_RADIUS = 0.025;
...
// Appends a colored sphere to the scene's geom buffer. Must be called
// AFTER mjv_updateScene each frame, since that call resets scn.ngeom to
// just the model's own geoms.
void addSphereMarker(mjvScene* scene, const Eigen::Vector3d& pos, double radius, const float rgba[4]) {
    if (scene->ngeom >= scene->maxgeom) return;
    mjtNum size[3] = {radius, radius, radius};
    mjtNum p[3]    = {pos.x(), pos.y(), pos.z()};
    mjtNum matI[9] = {1,0,0, 0,1,0, 0,0,1};
    mjv_initGeom(&scene->geoms[scene->ngeom], mjGEOM_SPHERE, size, p, matI, rgba);
    scene->ngeom++;
}
...
int main() {
    ...
    const float START_RGBA[4] = {0.15f, 0.85f, 0.25f, 1.0f}; // green
    const float GOAL_RGBA[4]  = {0.90f, 0.15f, 0.15f, 1.0f}; // red
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        addSphereMarker(&scn, root.p, MARKER_RADIUS, START_RGBA); // start pose, green
        addSphereMarker(&scn, p_goal, MARKER_RADIUS, GOAL_RGBA);  // goal pose, red
